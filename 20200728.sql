...
SQL 언어에 절차적 문법을 추가한 oracle 의 언어
SQL 은 집합적이지만 실생활에서 발생하는 요구사항을 처리 하기 위해서는 절차적 처리가 필요할 때가 있음
(복잡한 로직일때 - ex 연말정산 계산)
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('deptno : ' || deptno || ', dname : ' || dname); --java의 sysout
END;
/ --PL/SQL의 쿼리가 실직적으로 끝난다는 것을 알려주는 역할
...
JAVA 메소드 - public String 함수명(인자타입 인자명1, 인자타입2 인자명, ....)
...
EXEC printdept(20); --인자가 생겼으니 넣어줘야 한다.
...
==> 스칼라 변수, 변수 하나에 하나의 값만 할당 가능
    java 에서 변수 하나에 여러개의 값을 넣을 수 있는 자료형
    배열 ==> List, Map, Set
    
변수 ==> 컬럼의 값
 컬럼 ==> 행, 행 전체를 담을 수 잇는 변수
