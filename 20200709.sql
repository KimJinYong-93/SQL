...
SELECT customer.cid, cnm, a.pid, pnm, day, cnt
FROM(SELECT *
     FROM cycle
     WHERE cid = 1 
...
WHERE customer.cid = a.cid AND a.pid = product.pid;


SELECT cycle.cid,cycle.pid, cnm, pnm, day, cnt
...
1.5 테스트, 중간점검
2. 코드를 깨끗하게 ==> 리팩토링(코드 동작은그대로 유지한체 깔끔하게 정리하는 거)
...
특이점 : 1. WHERE 절에서 사용
        ...
        3. 비상호연관 서브쿼리, 상호연관 서브쿼리 둘 다 사용 가능하지만 주로 상호연관 서브쿼리[확인자]
           와 사용된다.
...
가로 확장(컬럼을 확장) : JOIN 
세로 확장(행을 확장) : 집합연산
                     집합연산을 하기 위해서는 연산에 참여하는 두개의 SQL(집합)이 동일한 컬럼 개수와
                     타입을 가져야한다.
...
SELECT empno eno, ename  --컬럼의 순서는 맞춰야 한다
FROM emp
WHERE empno IN (7369, 7566, 7499)
...
FROM emp
WHERE empno IN (7566, 7698)
ORDER BY ename; --ORDER BY 는 마지막에 써준다
...
SELECT *
FROM dept;
deptno가 90, dname이 ddit, loc가 daejeon인 데이터를 dept 테이블에 입력하는 쿼리 작성

INSERT INTO dept (deptno, dname, loc) 
          VALUES (90, 'ddit', 'daejeon');
