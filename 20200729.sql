...
정규화 이유 - 데이터 상태 이상을 방지

정규화(nomalization) 끝나고 나서 물리적인 고려 - 반정규화(de-nomalization)
...
dept테이블의 10번 부서의 부서번호랑, 부서이름을 pl/sql record type으로 생성된 변수에 값을 담아서 출력
...
TABLE TYPE 선언
TYPE 테이블 타입 이름 IS TABLE OF 행의 타입(ROW TYPE OR RECORD TYPE) INDEX BY BINARY_INTEGER
...
CURSOR : SELECT 문이 실행되는 메모리 상의 공간
         다량의 데이터를 변수에 담게되면 메모리 낭비가 심해져 프로그램이 정상적인 동작을 못할 수도 있음.
         한번에 모든 데이터를 인출하지 않고, 개발자가 직접 인출 단계를 제어 함으로써 변수에 모든 데이터
         를 담지 않고도 개발하는 것이 가능
         
CURSOR의 종류
묵시적 커서 : 커서 이름을 별도로 지정하지 않을 경우 ==> ORACLE 이 알아서 처리해줌
...
CURSOR 의 경우 반복문과 사용되는 일이 많기 때문에 PL/SQL 에서는 FOR LOOP 문과 함께 사용하는 문법을 지원
한다.
문법
...
emp 테이블에서 특정 부서에 속하는 사원의 사번과 이름을 출력하는 로직을 파라미터가
있는 커서를 활용하여 작성하는 실습
...
BEGIN
    FOR dt_row IN (SELECT dt FROM dt) LOOP
        EXIT WHEN i > 7;
        sum := sum + dt_row.dt(i) - dt_row.dt(i + 1);
        i := i + 1;
        DBMS.OUTPUT.PUT_LINE(sum);
    END LOOP;
END;
/
