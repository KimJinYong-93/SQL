...
3. 테이블 생성 이후, 객체수정 명령을 통해 제약조건을 추가


제약조건 생성방법 2번 - 테이블 생성시 컬럼 기술 이후 별도로 제약조건을 기술하는 방법
...
dept_test테이블에 deptno가 동일한 값을 갖는 INSERT 쿼리를 2개 생성하여 2개의쿼리가 정상적으로 동작하는지
테스트
...
[ UNIQUE 제약조건 ] - 해당 컬럼의 값이 다른 행에 나오지 않도록(중복되지 않도록) 데이터 무결성을 지켜주는
                 조건(ex - 사번, 학번)
...
/*  dname, loc 를 결합해서 중복되는 데이터가 없으면 됨
    ...
    아래는 부서명은 동일하지만 loc 정보가 다르기 때문에 dname, loc 조합은 서로 다른 데이터
    ddit, daejeon
    ddit, 대전    */
...
[ FOREIGN KEY - 참조키 ]
한 테이블의컬러므이 값의 참조하는 테이블의 컬럼 값중에 존재하는 값만 입력되도록 제어하는 제약조건
...
[ CHECK 제약조건 - 컬럼에 입력되는 값을 검증하는 제약 조건 ] 
    (ex - salary 컬럼(급여)이 음수가 입력되는 것은 부자연스러움
          ...
          직원 구분이 정직원, 임시직 2개가 존재할 때 다른 값이 들어오면 논리적으로 어긎남)
...
INSERT INTO emp_test VALUES (9999, 'brown', -500); --sal 값이 음수인 데이터 입력
...
테이블 컬럼 구조만 복사하고 싶을때 WHERE 절에 항상 FALSE 가 되는 조건을 기술하여 생성 가능
...
1. 신규 컬럼 추가
ALTER TABLE emp_test ADD (hp VARCHAR2(11)); --컬럼 순서는 바꿀 수 없음. 생성 전에 고려해야 함
SELECT *
FROM emp_test;
...
WHERE t.table_name = c.table_name
  AND c.table_name IN ('CUSTOMER', 'CYCLE', 'DAILY', 'PRODUCT');
  

ALTER TABLE emp ADD CONSTRAINT pk_emp_p PRIMARY KEY (empno);
ALTER TABLE dept ADD CONSTRAINT pk_dept_p PRIMARY KEY (deptno);
ALTER TABLE emp ADD CONSTRAINT pk_emp_f FOREIGN KEY (deptno) REFERENCES dept (deptno)
                                            ON DELETE CASCADE;
