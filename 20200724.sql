...
1. CONNECT BY LEVEL <, <= 정수
   - 시작행, 연결될 다음 행과의 조건이 없음
     ==> CROSS JOIN 과 유사
2. START WITH, CONNECT BY - 일반적인 계층 쿼리(일반 테이블과는 잘 쓰지 않는다 dual 테이블과 사용)
...
(SELECT empno, ename, hiredate, sal
FROM emp
ORDER BY sal DESC, hiredate) a) c,

(SELECT b.*, (ROWNUM-1) rn2
FROM 
(SELECT sal
FROM emp
...
SELECT empno, ename, sal, 
       SUM(sal) OVER (ORDER BY sal ROWS ) c_sum
FROM emp;
...
논리모델 - 시스템에서 필요로 하는 엔터티(테이블), 엔터티의 속성, 엔터티간의 관계를 기술
          데이터가 어떻게 저장될지는 관심사항이 아니다 ==> 물리모델에서 고려할 사항
          논리모델에서는 데이터의 전반적인 큰 틀을 설계
물리모델 - 논리모델을 갖고 해당 시스템이 사용할 데이터베이스를 고려하여 최적화된 테이블, 컬럼, 제약조건을
          설계하는 단계
...
관계차수 1-N, 1-1, N-N ==> 1:N 으로 변경할 대상
        수직바, 까마귀발
