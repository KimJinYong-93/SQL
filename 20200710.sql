...
방금 입력한 9999번 사원번호를 갖는 사원의 deptno와 job 컬럼의 값을 SMITH 사원의 deptno와
job 값으로 업데이트
...
한 사람의 트랜잭션이 다른사람의 트랜잭션에 어떤 영향을 미치는가
...
PHANTOM READ - LV2 에서는테이블에 존재하는 데이터에 대해 후행 트랜잭션에서 작업하지 못하도록
막을 수는 있지만 후행 트랜잭션에서 신규로 입력하는 데이터는 막을 수 없다.
즉 선행 트랜잭션에서 처음 읽은 데이터와 후행트랜잭션에서 신규입력 후 커밋한 이후에 조회한 데이터가
불일치 할 수 있다(없던 데이터가 갑자기 생성되는 현상)

INSERT INTO dept VALUES (99, '대덕', 'daejeon');

후행 트랜잭션에서 신규 입력 후 commit. 6개의 데이터가 조회됨
...
후행트랜잭션이 데이터를 입력, 수정 삭제하더라도 선행트랜잭션에서는 트랜잭션 시작 시점의 데이터가 보이
도록 보장
...
후행 트랜잭션에서dept 테이블에 데이터입력 후 commit;


DML(Date Manipulation[조작] Language) -  데이터를 다루는 SQL
...
--[ 오라클에서 제공하는 데이터 무결성을 지키기 위해 제공하는 제약조건 5가지(정확히는 4가지) ]
...
첫번째INSERT 구문에 의해 99번 부서는 dept_test 테이블에 존재
deptno 컬럼의 값이 99번인 데이터가 이미 존재하기 때문에 중복데이터로 입력이 불가능
INSERT INTO dept_test VALUES (99, 'ddit2', '대전');

현 시점에서 dept 테이블에는 deptno 컬럼에 PRIMARY KEY 제약조건이 걸려 있지 않은 상황
...
INSERT INTO dept_test VALUES (99, 'ddi2', '대전');
